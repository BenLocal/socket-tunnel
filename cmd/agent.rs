...
async fn main() -> anyhow::Result<()> {
    ...
    let mut backend = BackendService::new(SERVER);
    let cancel_clone = cancel.clone();
    tokio::spawn(async move { backend.start_service(cancel_clone).await });
...
impl BackendService {
    ...
    async fn start_service(&mut self, shutdown: CancellationToken) {
        loop {
    ...
    pub async fn connect_async(&self, shutdown: CancellationToken) -> anyhow::Result<()> {
